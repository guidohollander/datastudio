...
INNER JOIN SC_PERSONREGISTRATION_INDIVIDUAL i ON i.CASEID = p.ID
WHERE p.ID IN (
...
SELECT 
    ...
    ch.CASEID,
    CASE 
        WHEN p.ID IS NOT NULL THEN 'Valid - points to case ID=' + CAST(p.ID AS NVARCHAR(20))
        ELSE 'BROKEN - case not found'
    END AS Status
FROM CHANGES ch
LEFT JOIN SC_PERSONREGISTRATION_PROPERTIES p ON p.ID = ch.CASEID
WHERE ch.CHANGESRECORDID IN (
    SELECT CAST(NewPkValue AS BIGINT) FROM dbo.MigrationScenarioReplayMap
    WHERE ReplayRunID = @ReplayRunID AND TableName = 'CHANGES'
);
...
WHERE ReplayRunID = @ReplayRunID
  AND TableName IN ('CHANGES', 'CMFRECORD', 'SC_PERSONREGISTRATION_PROPERTIES')
