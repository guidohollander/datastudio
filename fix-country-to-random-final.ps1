...
if (-not $password) { throw "SQLPASSWORD not found in .env file" }

$sqlScript = @"
UPDATE f SET f.Notes = 'gen: random(1, 250)'
...
SELECT 'Updated ' + CAST(@@ROWCOUNT AS NVARCHAR(10)) + ' COUNTRY fields to random(1,250)' AS Result;
"@

Write-Host "Fixing COUNTRY generators..." -ForegroundColor Cyan
